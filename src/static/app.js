document.addEventListener("DOMContentLoaded", () => {
        ...
        const participantsHtml = details.participants.length > 0
          ? `<ul class="participants-list">${details.participants.map(email => `
              <li class="participant-item">
                ...
                <button type="button" class="participant-remove" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(email)}" aria-label="Remove ${email}">✕</button>
              </li>
            `).join("")}</ul>`
  ...
  activitiesList.addEventListener("click", async (event) => {
    ...
    try {
      ...
      showMessage("Unable to remove participant. Please try again.", "error");
